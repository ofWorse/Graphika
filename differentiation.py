...
def differential_2p_for_plot(equation, a, b, order=1, delta=0.05, precision=4):
    equation = sympy.sympify(equation)
    delta = sympy.core.Rational(delta).limit_denominator(1000)
    y_for_plot = list()
    x_for_sub = a + delta
...
def differential_3p_for_plot(equation, a, b, order=1, delta=0.05, precision=4):
    equation = sympy.sympify(equation)
    delta = sympy.core.Rational(delta).limit_denominator(1000)
    y_for_plot = list()
    x_for_sub = a + delta
...
def differential_5p_for_plot(equation, a, b, order=1, delta=0.05, precision=4):
    equation = sympy.sympify(equation)
    delta = sympy.core.Rational(delta).limit_denominator(1000)
    y_for_plot = list()
    x_for_sub = a + delta
